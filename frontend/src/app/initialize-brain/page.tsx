...
export default function InitializeBrainPage() {
  ...
  const handleRun = async () => {
    ...
    try {
      // Animate steps one-by-one with a brief visual delay
  ...
  return (
    <div className="bg-background min-h-screen">
      <header className="bg-background border-b-4 border-primary px-6 lg:px-10 py-4 sticky top-0 z-40">
        ...
        <p className="font-headline font-bold text-xs uppercase text-on-surface-variant mt-1">Complete system setup and configuration</p>
      </header>
        ...
        {running && (
          <div className="border-4 border-primary p-4 bg-primary-fixed neo-shadow">
            <div className="flex items-center gap-3">
              <span className="inline-block w-5 h-5 border-2 border-primary border-t-transparent loading-spinner"></span>
          ...
          >
            {running ? (
              <span className="flex items-center justify-center gap-3">
                <span className="inline-block w-5 h-5 border-2 border-white border-t-transparent loading-spinner"></span>
                Running Setup…
              </span>
